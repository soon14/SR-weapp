...
module.exports = options => {
    return async function merchantAdminParse(ctx, next) {
        ...
        if (!options.loginNoNeed.includes(queryPath)) {
            ...
            let adminInfo = await ctx.service.merchantAdmin.getAdminFromCache(loginKey, ip);
            if (adminInfo.id === undefined) {
                ctx.body = ctx.out.error(401, "登录超时或已被登出");
                return
            }
            ctx.setData('merchantId', adminInfo.merchantId);
            ctx.setData('adminId', adminInfo.id);
        }
        await next();
    };
};